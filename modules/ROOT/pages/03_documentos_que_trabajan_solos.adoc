...
Aplicamos niveles de título y generamos una tabla de contenido
automática.
...
Si todos los encabezados principales usan `Título 1`, podemos
modificar su apariencia una sola vez y el cambio se aplicará a todos.
...
Modificar una carta concreta no debería alterar la plantilla
corporativa.
...
Un formulario puede limitar el tipo de entrada que realiza el usuario
mediante:
...
[arabic]
. Transforma un documento manual en uno basado en estilos.
. Crea una plantilla corporativa.
. Genera 20 cartas personalizadas.
. Filtra solo clientes de Valencia.

== 🔥 6. Nueva incidencia

Solo deben recibir la comunicación quienes tengan renovación pendiente y
pertenezcan a dos provincias concretas.
