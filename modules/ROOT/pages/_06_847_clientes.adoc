...
* crear formularios;
* generar informes;
* distinguir cuándo conviene una base de datos frente a una hoja de
cálculo.
...
No usamos el nombre como clave porque pueden existir personas con el
mismo nombre.
...
Ejemplo: en CLIENTES, `email` es un campo y cada cliente es un
registro.
...
*1:N* +
Un cliente puede tener muchos pedidos, pero cada pedido pertenece a un
cliente.

*N:M* +
Un pedido puede contener muchos productos y un producto puede aparecer
en muchos pedidos.
...
Ejemplo: no podemos asignar un pedido al cliente `C999` si ese cliente
no existe.
...
*Informe:* presenta información preparada para lectura, impresión o
distribución.

== 🏋️ 5. Practica

[arabic]
. Diseña tablas para proveedores y compras.
. Crea relaciones.
. Consulta compras > 500 € del último mes.
. Crea un formulario de alta de proveedor.

== 🔥 6. Nueva incidencia

____
Necesito todos los pedidos pendientes de clientes de Valencia cuyo total
sea superior a 300 €.
____
