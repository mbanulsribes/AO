= Tema 8 · Necesitamos un vídeo para ayer

.CAOS, S.L.
[NOTE]
====
Hay 63 clips, varios audios, imágenes repetidas y ningún orden.
====

== 🚨 1. Incidencia inicial

*Objetivo:* montar y exportar un vídeo coherente.

== 🎯 2. Qué aprenderás

* comprender resolución, relación de aspecto, FPS, códec y contenedor;
* organizar un proyecto audiovisual;
* trabajar con línea de tiempo;
* realizar cortes;
* ajustar audio;
* insertar títulos;
* usar transiciones y efectos con criterio;
* exportar y comprobar el resultado.

== 🛠️ 3. Rescate guiado · Videotutorial

=== Paso 1. Conceptos

* resolución;
* relación de aspecto;
* FPS;
* códec;
* contenedor;
* compresión.

=== Paso 2. Organización

[source,text]
----
PROYECTO_VIDEO/
├── video/
├── audio/
├── imagen/
├── exportaciones/
└── proyecto/
----

=== Paso 3. Proyecto

Configura resolución y FPS adecuados antes de empezar.

=== Paso 4. Importación

Añade recursos.

=== Paso 5. Línea de tiempo

* pistas;
* clips;
* cortes;
* desplazamientos.

=== Paso 6. Montaje básico

Primero se construye la historia.

=== Paso 7. Audio

* niveles;
* música;
* voz;
* sincronización.

=== Paso 8. Títulos

* título inicial;
* rótulos;
* créditos.

=== Paso 9. Transiciones

Úsalas con intención, no entre todos los clips.

=== Paso 10. Efectos

Solo cuando tienen una función narrativa o correctiva.

=== Paso 11. Exportación

Comprueba:

* formato;
* resolución;
* calidad;
* peso;
* reproducción final.

== 📚 4. Lo que necesitas saber

=== 4.1 Resolución

La resolución del vídeo se expresa normalmente como ancho × alto.

Ejemplos:

[source,text]
----
1280 × 720
1920 × 1080
----

La elección depende del destino y del material original.

=== 4.2 Relación de aspecto

Describe la proporción entre ancho y alto.

Ejemplos habituales:

* 16:9;
* 9:16;
* 1:1.

Mezclar vídeos verticales y horizontales requiere decidir cómo se
adaptarán al proyecto.

=== 4.3 FPS

FPS significa fotogramas por segundo.

Un proyecto debe mantener una configuración coherente con el material y
el destino.

=== 4.4 Contenedor y códec

El *contenedor* es el archivo que agrupa vídeo, audio y otros datos.

Ejemplo: `.mp4`.

El *códec* es el método con el que se codifica y comprime el vídeo o
audio.

No son lo mismo.

=== 4.5 Proyecto frente a vídeo exportado

El proyecto guarda:

* decisiones de edición;
* referencias a archivos;
* cortes;
* efectos;
* títulos.

El vídeo final debe renderizarse/exportarse.

=== 4.6 Flujo de trabajo

[arabic]
. preproducción;
. montaje básico;
. ajustes;
. efectos/transiciones;
. títulos/audio;
. exportación.

=== 4.7 Audio

Un vídeo puede verse aceptable y ser difícil de seguir si el audio está
mal.

Comprueba:

* nivel de voz;
* música demasiado alta;
* silencios;
* sincronización.

=== 4.8 Transiciones y efectos

Más efectos no significa mejor vídeo.

Una transición debe ayudar a entender un cambio.

Un efecto debe tener una función, no utilizarse solo porque está
disponible.

=== 4.9 Exportación

Compara siempre:

* calidad;
* peso;
* compatibilidad;
* destino.

== 🏋️ 5. Practica

[arabic]
. Montaje de 30 segundos sin efectos.
. Añade títulos y créditos.
. Añade audio y ajusta niveles.
. Exporta a dos configuraciones y compara peso/calidad.

== 🔥 6. Nueva incidencia

Reduce el tamaño del vídeo sin que la pérdida resulte inaceptable para
el destino indicado.

== 🔒 7. Ticket cerrado · Evaluable

Recursos diferentes por alumno.

Se valora:

* narrativa;
* duración;
* audio;
* títulos;
* continuidad;
* formato de salida;
* especificaciones.

== ✅ 8. Comprueba que sabes…

* [ ] Entiendo resolución y relación de aspecto.
* [ ] Entiendo FPS.
* [ ] Distingo códec y contenedor.
* [ ] Sé organizar un proyecto.
* [ ] Sé montar clips en una línea de tiempo.
* [ ] Sé ajustar audio y títulos.
* [ ] Sé usar transiciones y efectos con criterio.
* [ ] Sé exportar y comprobar un vídeo.
