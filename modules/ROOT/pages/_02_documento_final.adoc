...
*Objetivo:* reconstruir un documento profesional, estable y fácil de
mantener.
...
Guardar bien implica saber *dónde* está el archivo y *qué versión*
estamos modificando.
...
El *formato de carácter* afecta a letras o palabras concretas: tipo de
letra, tamaño, negrita, color, etc.

El *formato de párrafo* afecta al bloque completo: alineación, sangrías,
interlineado, espaciado y tabulaciones.

Confundir ambos niveles provoca documentos difíciles de mantener.

=== 4.2 Espaciado: Enter no sustituye a la maquetación

Pulsar Enter muchas veces para bajar contenido crea huecos frágiles. Si
cambia el texto, todo se desplaza.
...
Son adecuadas para pequeñas alineaciones de texto, pero no sustituyen
una tabla cuando la información realmente está organizada en filas y
columnas.
...
*Salto de sección:* divide el documento en partes que pueden tener
configuraciones diferentes, por ejemplo orientación o encabezado.
...
Una tabla es adecuada cuando la información tiene estructura real de
filas y columnas.
...
[arabic]
. activa marcas de formato si necesitas detectar saltos o espacios;
. revisa ortografía;
. comprueba encabezados y pies;
. revisa saltos;
. genera el PDF;
. abre el PDF y compáralo con el original.
